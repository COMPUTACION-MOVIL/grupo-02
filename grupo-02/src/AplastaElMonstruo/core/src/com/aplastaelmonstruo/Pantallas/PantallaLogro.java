...
public class PantallaLogro implements Screen
{
    ...
    public  PantallaLogro(final MainGame juego, boolean segundaOpcion, int n) // Constructor de la pantalla logro. El segundo parametro es para saber si debemos o no crear un boton de siguiente nivel
    {
        ...
        organizadorLogro.add(botonMenu).width(MainGame.ANCHO_VIRTUAL/4).height(MainGame.ALTO_VIRTUAL/5).center();
        ...
        {
            organizadorLogro.add(botonSiguiente).width(MainGame.ANCHO_VIRTUAL / 4).height(MainGame.ALTO_VIRTUAL / 5).center();

            botonSiguiente.addListener(new ClickListener(){
                @Override
                public  void clicked (InputEvent event, float x, float y)
                {
                    // Dado el nivel que el usuario paso entonces
                    switch (nivel)
                    {
                        case(1):
        ...
        vistaLogro = new FitViewport(MainGame.ANCHO_VIRTUAL, MainGame.ALTO_VIRTUAL, camara); // Por ultimo, se inicializa la vista con unas dimensiones tentativas de 1280x720.
    }
    ...
    public void render(float delta)
    {
        ...
        juego.dibujadorPantalla.draw(fondoPantallaLogro, 0, 0, MainGame.ANCHO_VIRTUAL, MainGame.ALTO_VIRTUAL); // Aqui se dibuja el fondo y se ajusta a las dimensiones dadas
