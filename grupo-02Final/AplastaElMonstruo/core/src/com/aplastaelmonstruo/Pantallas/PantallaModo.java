...
import com.aplastaelmonstruo.Escenarios.*;
...
public class PantallaModo implements Screen
{
    ...
    public PantallaModo(final MainGame juego) // Constructor de la pantalla modos de juego
    {
        ...
        organizadorModo.row();
        organizadorModo.add(botonHistoria).width(MainGame.ANCHO_VIRTUAL/4).height(MainGame.ALTO_VIRTUAL/8).center();
        organizadorModo.row();
        organizadorModo.add(botonMultijugador).width(MainGame.ANCHO_VIRTUAL/4).height(MainGame.ALTO_VIRTUAL/8).center();
        organizadorModo.row();
        organizadorModo.add(botonSupervivencia).width(MainGame.ANCHO_VIRTUAL/4).height(MainGame.ALTO_VIRTUAL/8).center();
        organizadorModo.row();
        organizadorModo.add(botonVolver).width(MainGame.ANCHO_VIRTUAL/14).height(MainGame.ALTO_VIRTUAL/8).bottom().left().expandX();
        ...
        botonHistoria.addListener(new ClickListener(){
            ...
            {
                // Si oprime este boton entonces pasara a la pantalla de seleccion de niveles
                juego.setScreen(new PantallaSelectorNivel(juego));
            }
        });
        ...
        botonMultijugador.addListener(new ClickListener(){
            ...
            public  void clicked (InputEvent event, float x, float y)
            {
                // Si el usuario oprime este boton entonces

                // Si la musica del menu esta reproduciendose entonces
                ...
                // Y paso a la pantalla de multijugador
                juego.setScreen(new Multijugador(juego));
            }
        });
        ...
        botonSupervivencia.addListener(new ClickListener(){
            ...
            public  void clicked (InputEvent event, float x, float y)
            {
                ...
                // Y paso a la pantalla de supervivencia
                juego.setScreen(new Supervivencia(juego));
            }
        });
        ...
        vistaModo= new FitViewport(MainGame.ANCHO_VIRTUAL, MainGame.ALTO_VIRTUAL, camara); // Por ultimo, se inicializa la vista con unas dimensiones tentativas de 1280x720.
    }
    ...
    public void render(float delta)
    {
        ...
        juego.dibujadorPantalla.draw(fondoPantallaModo, 0, 0, MainGame.ANCHO_VIRTUAL, MainGame.ALTO_VIRTUAL); // Aqui se dibuja el fondo y se ajusta a las dimensiones dadas
